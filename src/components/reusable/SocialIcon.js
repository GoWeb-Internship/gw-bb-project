...
const SocialIcon = ({ data = [], language }) => {
  return (
    !!data.length && (
      ...
      >
        <ul className="text-slate-50 w-9 flex mb-9 mx-auto justify-center md:justify-start md:mb-0 md:absolute md:right-[5%] md:top-0 md:flex-col  lg:flex-row lg:relative lg:right-0">
          {data.map(item => {
            return (
              <li
                key={item.id}
